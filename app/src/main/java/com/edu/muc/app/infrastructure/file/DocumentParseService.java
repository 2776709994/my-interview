...
/**
 * 通用文档解析服务
 * 使用 Apache Tika 解析多种文档格式，提取文本内容，供知识库和简历模块共同使用。
 *
 * <p>解析配置要点：</p>
 * <ul>
 *   <li>PDF 禁用图片提取（setExtractInlineImages=false），避免扫描图与内嵌图片噪声</li>
 *   <li>PDF 按坐标排序文本（setSortByPosition=true），改善多栏简历的阅读顺序</li>
 *   <li>DOCX 等格式禁用嵌入资源解析（NoOpEmbeddedDocumentExtractor），不解析图片与附件</li>
 *   <li>BodyContentHandler 只取正文并限制最大 5MB</li>
 *   <li>解析后经 {@link TextCleaningService} 正则清洗去噪</li>
 * </ul>
 */
@Slf4j
@Service
public class DocumentParseService {
    ...
    public String parseContent(MultipartFile file) {
        ...
        try (InputStream inputStream = file.getInputStream()) {
            ...
            throw new BusinessException("PARSE_ERROR", "文件解析失败: " + e.getMessage());
        }
    }
     ...
     * 3. 配置 PDFParserConfig，关闭图片提取并按位置排序文本
