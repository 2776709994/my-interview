...
export default function InterviewDetailPanel({ interview }: InterviewDetailPanelProps) {
  ...
  const [expandedQuestions, setExpandedQuestions] = useState<Set<number>>(() => {
    ...
    if (interview.questionDetails) {
      interview.questionDetails.forEach((_, idx) => allIndices.add(idx));
    }
    return allIndices;
  });
  ...
  return (
    ...
    >
      ...
      {/* 问答记录详情 */}
        <QuestionsSection
        questionDetails={interview.questionDetails || []}
        referenceAnswers={interview.referenceAnswers || []}
      ...
      <div className="space-y-4">
        {questionDetails.map((detail, idx) => {
          ...
          const referenceAnswer = referenceAnswers.find(
            (ref: any) => ref.questionIndex === detail.questionIndex
          );
    ...
    >
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className={`font-semibold ${getScoreColor(score, [80, 60])}`}>
            得分: {score}
          </span>
        </div>
          <motion.svg
      ...
      <div className="px-5 pb-2">
        <p className="text-slate-800 dark:text-white font-medium leading-relaxed">{detail.question}</p>
      </div>
            ...
            <div className="px-5 pb-5 space-y-4">
              ...
              <div className="bg-slate-50 dark:bg-slate-700/50 rounded-xl p-4">
                ...
                <p className={`leading-relaxed ${
                  !detail.userAnswer || detail.userAnswer === '' 
                    ? 'text-red-500 font-medium'
                      : 'text-slate-700 dark:text-slate-300'
                }`}>
                  "{detail.userAnswer || '(未回答)'}"
                </p>
              </div>
              ...
              {detail.feedback && (
                <div>
                  ...
                  <p className="text-slate-700 dark:text-slate-300 leading-relaxed pl-6">{detail.feedback}</p>
                </div>
              )}
              ...
              {referenceAnswer?.referenceAnswer && (
                  <div
                    ...
                    <div
                        className="text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-line">{referenceAnswer.referenceAnswer}</div>
                    {referenceAnswer.keyPoints && referenceAnswer.keyPoints.length > 0 && (
                      <div className="mt-3">
                        <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">关键点：</p>
                        <ul className="list-disc list-inside space-y-1">
                          {referenceAnswer.keyPoints.map((point: string, i: number) => (
                            <li key={i} className="text-sm text-slate-600 dark:text-slate-300">{point}</li>
                          ))}
                        </ul>
                      </div>
                    )}
